...
def exact_solution(xt, epsilon):
    x = xt[:, 0:1]  
    t = xt[:, 1:2]  
    v = (torch.exp(-x / epsilon) + torch.exp(-(torch.tensor(1.0, dtype=torch.float64, device=device) - x) / epsilon)) / (1.0 + torch.exp(-torch.tensor(1.0, dtype=torch.float64, device=device) / epsilon)) - 1.0
    return -torch.exp(torch.tensor(2.0, dtype=torch.float64, device=device) - t) * v
...
def f(xt, epsilon):
    ...
    u_t = -u
    u_xx = -torch.exp(torch.tensor(2.0, dtype=torch.float64, device=device) - t) / (epsilon**2) * ((torch.exp(-x / epsilon) + torch.exp(-(torch.tensor(1.0, dtype=torch.float64, device=device) - x) / epsilon)) / (1.0 + torch.exp(-torch.tensor(1.0, dtype=torch.float64, device=device) / epsilon)))
    return u_t - epsilon * u_xx + (1.0 + x * torch.exp(-t)) * u
...
def train_network(net, epsilon, f, num_epochs=500, early_stop_threshold=1e-3, T=1.0):
    ...
    for epoch in range(num_epochs):
        ...
        # 计算 PDE 残差
        pde_res = u_t - epsilon * u_xx + (1.0 + xt_i[:, 0:1] * torch.exp(-xt_i[:, 1:2])) * u_i - f(xt_i, epsilon)
...
def compute_pde_operator(net, xt, epsilon, explicit_bases_func, num_explicit_bases):
    ...
    for j in range(basis.shape[1]):
        ...
        # 计算 PDE 算子
        term = 1.0 + xt[:, 0:1] * torch.exp(-xt[:, 1:2])
        L_basis_j = basis_j_t - epsilon * basis_j_xx + term * basis_j
