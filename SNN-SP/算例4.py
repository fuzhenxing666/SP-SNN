...
def exact_solution(xy, epsilon):
    ...
    r_plus = (-1 + torch.sqrt(torch.tensor(1) + 4*epsilon**2*pi**2)) / (2*epsilon)
    r_minus = (-1 - torch.sqrt(torch.tensor(1) + 4*epsilon**2*pi**2)) / (2*epsilon)
    
    denominator = epsilon * torch.pi**2 * (torch.exp(r_plus) - torch.exp(r_minus))
    A = (torch.exp(r_minus) - 1) / denominator
    B = (1 - torch.exp(r_plus)) / denominator
    
    return (A*torch.exp(x*r_plus) + B*torch.exp(x*r_minus) + 1/(epsilon*torch.pi**2)) * torch.sin(torch.pi*y)
...
def explicit_bases(xy, epsilon, num_explicit_bases):
    ...
    bases = [torch.exp(-x / epsilon) * torch.sin(torch.pi * y), 
             ]  
